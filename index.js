...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5573981044096:+55 (73) 98104-4096\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '!'
blocked = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '🧐 rlx vagabundo tô fazendo o k4rlh 🧐',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo 𝐈𝐓𝐀𝐂𝐇𝐈! ❌',
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 𝐈𝐓𝐀𝐂𝐇𝐈 Bot 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ falhou, tenta aibdi nvo kkkk 🤝😔`)
							})
							.on('end', function () {
								console.log('Finish')
