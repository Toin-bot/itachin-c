...
axios.get(`https://st4rz.herokuapp.com/api/yta?url=${teks}`).then((res) => {
    let hasil = `aqui está o download do seu áudio 😎 hasilnya\n👇👇👇👇👇👇👇👇👇\n\nJudul: ${res.data.title}\n\nUkuran audio: ${res.data.filesize}\n\nLink: ${res.data.result}`;
...
conn.sendMessage(id, ' Thanks Telah Menggunakan BOT *NINEM* ,);
}
else if (text == '#info'){
conn.sendMessage(id, ' Thanks Telah Menggunakan BOT *BOT NINEM* t);
}
       ...
       if (caption == '#ocr')
       {
           ...
           readTextInImage(img)
               .then(data => {
                   console.log(data)
                   conn.sendMessage(id, ' SEDANG DIPROSES', MessageType.text)
                   conn.sendMessage(id, `${data}\n*NI NI DOG ANJIM🗿*`, MessageType.text);
               })
               .catch(err => {
                   console.log(err)
               })
