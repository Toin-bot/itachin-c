...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ *BEM VINDO (a) este GRUPO ${mdata.subject}* ] \n*___________________________*\n@${num.split('@')[0]} Intro/Dikick!!! \nNome : \nIdade : \nAskot : \nGenero : \nJa Conhece As Regras Sim/Nao?: \n*Numero Fake Foto No Perfil!!!* \n nome instagram? \n Salve Pelo Admin! \n *___________________________*\nEspero Que Se Sinta Em Casa 🏡!`
				...
				teks = `ADEUS... @${num.split('@')[0]}👋* \n_Cuidado Por Ai , Foi Um Prazer Sua Presenca Neste Grupo_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬ AGUARDE ❭* _Se nao fazer ,tente de Novo...⏳_',
				...
				only: {
					...
					Badmin: '*Este Comando so pode ser usado se o Bot for admin , nao sou Mágico para remover sem administração!*'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5573981044096@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["55739810440964@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["5573981044096@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Esta e a lista de números bloqueados:\n'
			...
			case 'owner':
				...
				if (!isGroup) return reply(mess.only.group)
					...
					teks += `*Nome do grupo :* ${groupName}\n*Descricao :* ${groupDesc}\n*Total Admin :* ${groupAdmins.length}\n*Numero De Membros :* ${groupMembers.length}`
					no = 0
                ...
                client.sendMessage(from, 'Succeso, apagou a Descricao do Grupo', text, {quoted: mek})
                break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Qual código de linguagem?', text, {quoted: mek})
	            ...
	            case 'setpp':
                    ...
                    reply('Sucesso mudou icone do Grupo')
                    break
                case 'apakah':
					apakah = body.slice(1)
               ...
               case 'infogempa':
				...
				case 'minato':
					...
					break
				case 'boruto':
					if (!isAnime) return reply('❌ *Deve Ativar Modo Anime* ❌')
				...
				case 'sasuke':
					...
					break
				case 'sakura':
					if (!isAnime) return reply('❌ *Ative Modo Anime* ❌')
				...
				case 'kaneki':
					...
					break
				case 'toukachan':
					if (!isAnime) return reply('❌ *Primeiro Ative Modo Anime* ❌')
                ...
                case 'indohot':
                   ...
                   break
				case 'ytmp4':
					if (args.length < 1) return reply('Onde esta o url um?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Cade o Url um?')
				...
				case 'tiktokstalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('Provavelmente username esta invalido')
					}
					break
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Sobre o que voce quer escrever?')
					tulis = body.slice(6)
					reply(mess.wait)
				...
				break
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(` *comando Recebido, agora o grupo esta aberto todos podem falar* `)
			    ...
			    case 'kick':
					...
					if (mentioned.length > 1) {
						teks = 'Pedido recebido , suma daqui 🚴 :\n'
						...
						mentions(`Pedido recebido, removido com sucesso, vaza daqui 🏌️ : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'edotense':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o membro que deseja Remover @!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o destino que você deseja que seja um administrador!!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'listadmins':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ marque s figurinha pra virar foto  ❌')
					reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('Modo simi esta ativo')
						samih.push(from)
				...
				case 'openanime':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isAnime) return reply('Modo anime está ativo')
						anime.push(from)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ja esta ativado')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sucesso agora quem entrar darei Boas vindas ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sucesso Desativado  recurso Boas vindas neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
				case 'clone':
				...
				case 'hidetag2':
					if (!isGroup) return reply(mess.only.group)
					if (!isadminbot) return reply('Quem e voce ?')
					var value = body.slice(9)
					var group = await client.groupMetadata(from)
					var member = group['participants']
					var mem = []
				...
				case 'setpp3':
                    ...
                    break
                case 'bc3':
					if (!isfrendsowner) return reply('Quem e voce ?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
